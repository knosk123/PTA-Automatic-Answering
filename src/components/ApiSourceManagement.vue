...
function loadConfig() {
  chrome.storage.local.get(['apiSources'], (result) => {
    if (Array.isArray(result.apiSources) && result.apiSources.length > 0) {
      ...
      // 添加默认 API 源（限定 4 个默认源：硅基流动、阿里云百炼、小米 MiMo、MiniMax-M3）
      // 默认启用：用户只需填 API key 即可使用（不需要手动 enable）
...
async function checkModel(model, apiKey, apiUrl) {
  ...
  try {
    // v3 改造：对所有源统一使用 chat/completions 测试（不再用 GET /models/{model}，因中转站不一定支持）
...
<template>
  <div class="api-source-management">
    ...
    <div class="left-panel">
      ...
      <!-- v3 改造：移除"添加 API 源"按钮（限定 4 个默认源） -->
    </div>
    ...
    <!-- 添加 API 源弹窗 -->
    <!-- v3 改造：已移除"添加 API 源"弹窗（限定 4 个默认源） -->
